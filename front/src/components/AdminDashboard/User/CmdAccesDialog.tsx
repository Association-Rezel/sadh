...
import {
  CommandeAccesInfo as CommandeAccesInfo,
  User,
...
export default function CmdAccesDialog({
  open,
  onClose,
  user,
}: CmdAccesDialogProps) {
  ...
  useEffect(() => {
    ...
    Api.fetchONT(user.id).then((ont: ONTInfo) => {
      ...
      reset({
        ...
        date_installation: dayjs(user.membership.appointment.slot.start).format(
          "YYYYMMDD HH:mm",
        ),
  ...
  return (
    <Dialog onClose={onClose} open={open} maxWidth="sm" fullWidth={true}>
      <form onSubmit={handleSubmit(sendData)}>
        ...
        {!hasONT && (
          <DialogTitle>
            Vous devez d'abord assigner un ONT et valider un créneau de
            rendez-vous
          </DialogTitle>
        )}
        <DialogActions>
