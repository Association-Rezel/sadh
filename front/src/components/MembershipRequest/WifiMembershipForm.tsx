...
export default function WifiMembershipForm() {
  ...
  const onSubmitMembership = async (event: FormValues) => {
    try {
      const realSSID = allSSIDs.find(
        (ssid) => ssid.toLowerCase() === "rezel-" + event.ssid.toLowerCase(),
      );
      if (!realSSID) {
        alert(
          "Erreur lors de la demande d'abonnement. Le réseau Wi-Fi n'existe pas.",
        );
        return;
      }
  ...
  return (
    <div>
      ...
      <form
        className="container mt-16 max-w-2xl"
        ...
        <div className="flex flex-col items-start">
          <div className="mt-8 mb-12">
            ...
            <Typography variant="body2" color="text.secondary" align="left">
              Merci d'indiquer le réseau Wi-Fi Rezel-xxx que tu reçois le mieux
              de chez toi.
              <br />
              Nous créerons ensuite un nouveau Wi-Fi complètement séparé sur le
              même équipement physique.
              <br />
              Tu auras ton propre réseau et ta propre adresse IP publique
              complètement séparés du réseau que tu indiques ci-dessous.
            </Typography>
          </div>
          {loadingSSIDs && <CircularProgress />}
          {errorSSIDs && (
            <Alert severity="error">
              Erreur lors du chargement des réseaux Wi-Fi, veuillez contacer
              fai@rezel.net
            </Alert>
          )}
                ...
                <Controller
                  ...
                  rules={{
                    ...
                    validate: (value) =>
                      allSSIDs.find(
                        (ssid: string) =>
                          ssid.toLowerCase() === "rezel-" + value.toLowerCase(),
                      ) != null,
                  }}
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <FormControl required sx={{ minWidth: 240 }}>
              <InputLabel id="residence-label">
                Nom de votre résidence
              </InputLabel>
              <Controller
                name="residence"
                control={control}
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    onChange={onChange}
                  >
                    {Object.values(Residence).map((key) => (
        ...
        <div className="mt-8">
          ...
          <ul className={"flex flex-col text-left gap-4 pl-4 mt-6"}>
            <li>
              ...
              <Typography variant="body2" color="text.secondary" align="left">
                Le premier mois d'abonnement est à régler dès maintenant. Une
                fois que nous aurons confirmé la réception du paiement, nous
                activerons un nouveau réseau Wi-Fi la box que vous indiquée plus
                haut. Vous recevrez alors un mail de confirmation.
              </Typography>
            </li>
            ...
            <li>
              ...
              <Typography variant="body2" color="text.secondary" align="left">
                Vous devez régler votre cotisation pour un an (sauf si vous êtes
                déjà membre). Être membre adhérent de Rezel est obligatoire pour
                bénéficier du service FAI. Votre adhésion commencera le jour de
                la signature de votre contrat. Elle est à renouveler chaque
                année à la date anniversaire de votre adhésion.
              </Typography>
            </li>
          </ul>
        </div>
