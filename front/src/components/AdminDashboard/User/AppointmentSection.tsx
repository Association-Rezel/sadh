...
export default function AppointmentSection({
  ...
  const syncAvailabilitySlots = () => {
    Api.updateUser(user.id, {
      ...
      .catch((error) => {
        alert(
          "Erreur lors de la synchronisation des créneaux. Veuillez essayer de recharger la page. Message d'erreur : " +
            error.message,
        );
      });
  };
  ...
  const syncAppointment = () => {
    Api.updateMembership(user.id, {
      ...
      .catch((error) => {
        alert(
          "Erreur lors de la synchronisation des rendez-vous. Veuillez essayer de recharger la page. Message d'erreur : " +
            error.message,
        );
      });
  };
  ...
  const onAddSlot = () => {
    ...
    if (
      [...user.availability_slots, user.membership?.appointment?.slot].filter(
        (slot) => isSameSlot(slot, newSlot),
      ).length > 0
    )
        ...
        {user.availability_slots !== null && (
          <>
            ...
            <div style={{ marginTop: "10px", marginBottom: "10px" }}>
              ...
              <Button variant="contained" color="primary" onClick={onAddSlot}>
                Ajouter le créneau
              </Button>
            </div>
            ...
            <div>
              <TextField
                ...
                className="bg-white"
                minRows={3}
                {...registerToMembershipUpdateForm("comment")}
              />
            </div>
            <div className="mt-5">
              ...
              <TextField
                ...
                size="small"
                fullWidth
                {...registerToMembershipUpdateForm("erdv_id")}
              />
            </div>
          </>
        )}
