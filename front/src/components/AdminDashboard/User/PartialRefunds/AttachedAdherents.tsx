...
const AttachedAdherents = ({ user, setUser }: { user: User; setUser: any }) => {
  ...
  const { formState, control, handleSubmit, reset, getValues } = useForm<{
    adherents: DayJSAwareAttachedWifiAdherent[];
  }>();

  useEffect(() => {
    reset({
      adherents:
        user.membership?.attached_wifi_adherents.map((adherent) => ({
          ...adherent,
  ...
  const handleAddRow = () => {
    reset((prev) => ({
      adherents: [
        ...prev.adherents,
        { from_date: dayjs(), to_date: null, user_id: "", comment: "" },
      ],
    }));
  };
  ...
  return (
    <div className="mt-5">
      <div className="flex flex-col items-start mt-4">
        ...
        {formState.errors.adherents && (
          <Alert severity="error">
            La date de début et la UUID doivent être remplis pour toutes les
            entrées
          </Alert>
        )}
        <TableContainer component={Paper}>
          <Table size="small" aria-label="a dense table">
            ...
            <TableBody>
              {getValues().adherents &&
                getValues().adherents.map((adherent, index) => (
                  <TableRow
                    key={
                      adherent.user_id +
                      adherent.from_date.toISOString() +
                      adherent.to_date?.toISOString()
                    }
                  >
                    <TableCell>
                      {isEditing ? (
                        ...
                        >
                          <Controller
                            ...
                            rules={{ required: true }}
                            render={({ field }) => (
                              <DatePicker {...field} label="Depuis le" />
                            )}
                          />
                        </LocalizationProvider>
                      ) : (
                    ...
                    <TableCell>
                      {isEditing ? (
                        ...
                        (adherent.to_date?.format("DD/MM/YYYY") ?? (
                          <Chip label="En cours" />
                        ))
                      )}
                    </TableCell>
