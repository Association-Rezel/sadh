...
class Api {
  ...
  parseUser(data: any): User {
    if (data?.availability_slots) {
      data.availability_slots = data.availability_slots.map((slot: any) =>
        this.parseAppointmentSlot(slot),
      );
    }

    if (data?.membership?.appointment) {
      data.membership.appointment = this.parseAppointment(
        data.membership.appointment,
      );
    }

    if (data?.membership?.attached_wifi_adherents) {
      data.membership.attached_wifi_adherents =
        data.membership.attached_wifi_adherents.map((adherent: any) =>
          this.parseAttachedWifiAdherent(adherent),
        );
    }
  ...
  async generateVerificationCode(): Promise<User> {
    return this.parseUser(
      await this.myFetcher("/users/me/generateVerificationCode"),
    );
  }

  async checkVerificationCode(code: string): Promise<User> {
    return this.parseUser(
      await this.myFetcher("/users/me/checkVerificationCode?code=" + code),
    );
  }
  ...
  async submitMyMembershipRequest(request: MembershipRequest): Promise<User> {
    return this.parseUser(
      await this.myFetcher("/users/me/membershipRequest", request),
    );
  }

  async fetchAppointmentSlots(
    weekOffset: number,
  ): Promise<AppointmentSlot[][]> {
    const data = await this.myFetcher<object[][]>(
      "/appointments/weekSlots?weekOffset=" + weekOffset,
    );
    return data.map((week: any) =>
      week.map((slot: any) => this.parseAppointmentSlot(slot)),
    );
  }

  async updateMyAvailabilities(
    availabilities: AppointmentSlot[],
  ): Promise<User> {
    return this.parseUser(
      await this.myFetcher("/users/me/availability", availabilities),
    );
  }

  async fetchMyUnet(): Promise<UnetProfile | null> {
    return await this.fetchOrDefault<UnetProfile | null>(
      "/users/me/unet",
      null,
    );
  }
  ...
  async fetchUser(user_id: string): Promise<User | null> {
    return this.parseUser(
      await this.fetchOrDefault<User | null>("/users/" + user_id, null),
    );
  }
  ...
  async fetchONT(user_id: string): Promise<ONTInfo | null> {
    return await this.fetchOrDefault<ONTInfo | null>(
      "/users/" + user_id + "/ont",
      null,
    );
  }
  ...
  async registerONT(user_id: string, register: RegisterONT): Promise<ONTInfo> {
    return await this.myFetcher<ONTInfo>(
      "/users/" + user_id + "/ont",
      register,
    );
  }
  ...
  async updateUser(user_id: string, update: Partial<User>): Promise<User> {
    return this.parseUser(
      await this.myFetcher(`/users/${user_id}`, update, "PATCH"),
    );
  }
  ...
  async updateMembership(
    ...
    return this.parseUser(
      await this.myFetcher(`/users/${user_id}/membership`, membership, "PATCH"),
    );
  }

  async deleteMembership(user_id: string): Promise<User> {
    return this.parseUser(
      await this.myFetcher(`/users/${user_id}/membership`, null, "DELETE"),
    );
  }

  async fetchUserBox(user_id: string): Promise<Box | null> {
    return await this.fetchOrDefault<Box | null>(
      "/users/" + user_id + "/box",
      null,
    );
  }

  async registerUserBox(
    user_id: string,
    box_type: string,
    ptah_profile: string,
    mac_address: string,
  ...
  async sendCRMiseEnService(info: CRMiseEnService): Promise<Response> {
    return await this.myFetcher(
      "/nix/generer_cr_mise_en_service",
      info,
      "POST",
      "raw",
    );
  ...
  async downloadPtahImage(mac: string, ptah_profile: string): Promise<void> {
    await this.myFetcher(
      `/ptah/build/${mac}/${ptah_profile}`,
      null,
      "POST",
      "download",
      "ptah.bin",
  ...
  async fetchNextMembershipStatus(
    user_id: string,
  ): Promise<StatusUpdateInfo | null> {
    return await this.fetchOrDefault<StatusUpdateInfo | null>(
      "/users/" + user_id + "/next-membership-status",
      null,
    );
  }
  ...
  async updateMembershipStatus(
    ...
    return this.parseUser(
      await this.myFetcher(
        `/users/${user_id}/next-membership-status?next_status=${status}`,
        null,
        "POST",
      ),
    );
  ...
  async fetchBoxByUnetID(main_unet_id: string): Promise<Box | null> {
    return await this.fetchOrDefault<Box | null>(
      "/devices/box/by_unet_id/" + main_unet_id,
      null,
    );
  }

  async createUnetOnBox(
    id: string,
    macAddress: string,
    isTelecomian: boolean,
  ): Promise<Box> {
    return await this.myFetcher<Box>(
      `/users/${id}/unet?mac_address=${macAddress}&telecomian=${isTelecomian}`,
      null,
      "POST",
    );
  }
  ...
  async generateNewContract(user_id: string): Promise<void> {
    await this.myFetcher(
      `/users/${user_id}/generate_new_contract`,
      null,
      "POST",
    );
  }
  ...
  async refreshContract(user_id: string): Promise<User> {
    return this.parseUser(
      await this.myFetcher(`/documenso/refresh/${user_id}`, null, "POST"),
    );
  }

  async payUserPartialRefunds(user_id: string): Promise<void> {
    await this.myFetcher(
      `/users/${user_id}/pay-user-partial-refunds`,
      null,
      "POST",
    );
  }
  ...
  async fetchAllScholarshipStudents(): Promise<User[]> {
    const users = await this.fetchOrDefault<User[]>(
      "/users/scholarship-student",
      [],
    );
    return users.map((user: any) => this.parseUser(user));
  }
  ...
  async transferUnet(
    ...
    return await this.myFetcher(
      `/net/transfer-unet/${unet_id}/to/${target_mac_address}`,
      null,
      "POST",
    );
  }
  ...
  async updateUnetWifiStatus(user_id: string, disabled: boolean): Promise<Box> {
    return await this.myFetcher<Box>(
      `/users/${user_id}/unet/disabled?disabled=${disabled}`,
      null,
      "PATCH",
    );
  }

  async updateBoxMacAddress(
    mac_address: string,
    new_mac_address: string,
  ): Promise<Box> {
    return await this.myFetcher<Box>(
      `/devices/box/${mac_address}/mac/${new_mac_address}`,
      null,
      "PATCH",
    );
  }

  async updateBoxPtahProfile(
    mac_address: string,
    new_ptah_profile: string,
  ): Promise<Box> {
    return await this.myFetcher<Box>(
      `/devices/box/${mac_address}/ptah_profile/${new_ptah_profile}`,
      null,
      "PATCH",
    );
  }
  ...
  async forceOntRegistration(serial_number: string): Promise<ONTInfo> {
    return await this.myFetcher<ONTInfo>(
      `/devices/ont/${serial_number}/register_in_olt`,
      null,
      "POST",
    );
  }
  ...
  async fetchAllONTSummary(): Promise<string> {
    return await this.fetchOrDefault<string>(
      "/net/get-all-ont-summary",
      "Cannot fetch",
    );
  }
  ...
  async createIpamLog(message: string, source: string): Promise<void> {
    await this.myFetcher(
      `/logging/ipam?message=${message}&source=${source}`,
      null,
      "POST",
    );
  }

  async transferDevices(
    user_id: string,
    target_user_id: string,
  ): Promise<void> {
    await this.myFetcher(
      `/users/${user_id}/transfer-devices?target_user_id=${target_user_id}`,
      null,
      "POST",
    );
  }
  ...
  async fetchAllUsersOnBox(mac_address: string): Promise<User[]> {
    const users = await this.fetchOrDefault<object[]>(
      `/devices/box/${mac_address}/users`,
      [],
    );
    return users.map((user: any) => this.parseUser(user));
  }
  ...
  async updatePartialRefund(
    partial_refund: PartialRefund,
  ): Promise<PartialRefund> {
    return this.parsePartialRefund(
      await this.myFetcher("/partial-refunds", partial_refund, "PATCH"),
    );
  }

  async computePartialRefunds(): Promise<object> {
    return await this.myFetcher<object>(
      "/partial-refunds/compute",
      null,
      "POST",
    );
  }
  ...
  async checkAuthStatusForPath(
    path: "user" | "admin",
  ): Promise<JwtUserData | null> {
    const response = await this.myFetcher<AuthStatusResponse<JwtUserData>>(
      `/auth/status/${path}`,
    );
  ...
  async fetchMyArchivedInvoices(): Promise<any[]> {
    return await this.fetchOrDefault<any[]>(
      "/payments/me/archived-invoices",
      [],
    );
  }

  async pollDolibarrPaymentStatus(): Promise<{ paid: boolean }> {
    return await this.myFetcher<{ paid: boolean }>(
      "/payments/me/dolibarr-payment-status",
    );
  }

  async createSelectivePayment(
    items: string[],
  ): Promise<{ payment_url: string | null; invoice_id: number | null }> {
    return await this.myFetcher<{
      payment_url: string | null;
      invoice_id: number | null;
    }>("/payments/me/selective-payment", { items }, "POST");
  }

  async fetchInvoicePaymentUrl(
    invoiceId: string,
  ): Promise<{ payment_url: string | null }> {
    return await this.myFetcher<{ payment_url: string | null }>(
      `/payments/me/invoices/${invoiceId}/payment-url`,
    );
  }
  ...
  async createAdvancePayment(
    ...
    return await this.myFetcher<{
      payment_url: string | null;
      invoice_id: number | null;
    }>(`/payments/me/advance-payment?num_months=${numMonths}`, null, "POST");
  }

  async downloadInvoicePdf(invoiceId: string): Promise<void> {
    await this.myFetcher(
      `/payments/me/invoices/${invoiceId}/pdf`,
      null,
      "GET",
      "download",
      "facture.pdf",
  ...
  async checkInvoicePaid(invoiceId: string): Promise<{ paid: boolean }> {
    return await this.myFetcher<{ paid: boolean }>(
      `/payments/me/invoices/${invoiceId}/status`,
    );
  }

  async cancelInvoice(invoiceId: string): Promise<void> {
    await this.myFetcher(
      `/payments/me/invoices/${invoiceId}/cancel`,
      null,
      "POST",
    );
  }

  async fetchSubscriptionInfo(): Promise<{
  ...
  async fetchUserSubscriptionInfo(userId: string): Promise<{
    total_months_paid: number | null;
    subscription_end: number | null;
    membership_end: number | null;
  }> {
  ...
  async fetchOverdueUsers(): Promise<{
    subscription_expired: OverdueEntry[];
    cotisation_expired: OverdueEntry[];
  }> {
    return await this.myFetcher<{
      subscription_expired: OverdueEntry[];
      cotisation_expired: OverdueEntry[];
  ...
  async remindOverdueSubscription(userId: string): Promise<{
    user_id: string;
    invoice_id: number | null;
    mail_sent: boolean;
  }> {
    return await this.myFetcher<{
      ...
      invoice_id: number | null;
      mail_sent: boolean;
    }>(`/overdue/${userId}/remind-subscription`, null, "POST");
  }

  async remindOverdueCotisation(userId: string): Promise<{
    user_id: string;
    invoice_id: number | null;
    mail_sent: boolean;
  }> {
    return await this.myFetcher<{
      ...
      invoice_id: number | null;
      mail_sent: boolean;
    }>(`/overdue/${userId}/remind-cotisation`, null, "POST");
  }

  async remindAllOverdueSubscription(): Promise<
    { user_id: string; invoice_id: number | null; mail_sent: boolean }[]
  > {
    return await this.myFetcher<
      { user_id: string; invoice_id: number | null; mail_sent: boolean }[]
    >("/overdue/remind-all-subscription", null, "POST");
  }

  async remindAllOverdueCotisation(): Promise<
    { user_id: string; invoice_id: number | null; mail_sent: boolean }[]
  > {
    return await this.myFetcher<
      { user_id: string; invoice_id: number | null; mail_sent: boolean }[]
    >("/overdue/remind-all-cotisation", null, "POST");
  }
