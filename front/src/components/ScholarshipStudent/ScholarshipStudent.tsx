    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-row gap-4">
        ...
        <ConfirmableButton
          confirmationText={
            <p>
              Êtes-vous sûr de vouloir réinitialiser tous les étudiants
              boursiers ?
            </p>
          }
          onConfirm={resetAll}
      ...
      {students && students.length > 0 && (
        <div>
          <Typography variant="h6">Étudiants boursiers</Typography>
          <List>
            {students.map((student) => (
              <ListItem key={student.id}>
                ...
                <IconButton
                  onClick={() => {
                    Api.updateUser(student.id, { scholarship_student: false })
                      .then(() => {
                        setStudents(
      ...
      {students && students.length === 0 && (
        <Typography variant="body1">Aucun étudiant boursier trouvé.</Typography>
      )}
    </div>
  );
}
