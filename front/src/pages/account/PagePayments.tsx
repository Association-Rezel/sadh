...
export default function PagePayments() {
  ...
  const [pendingAdvanceInvoiceId, setPendingAdvanceInvoiceId] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    const handler = () => {
      navigator.sendBeacon(
        `/api/payments/me/invoices/${pendingAdvanceInvoiceId}/cancel`,
      );
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-10 px-6">
        <img src={logoRezel} alt="logo Rezel" style={{ height: 100 }} />
        <Typography
          className="max-w-xl"
          variant="h4"
          align="center"
  ...
  const canPayAdvance =
    totalMonthsPaid !== null && totalMonthsPaid > 0 && maxAdvanceMonths > 0;

  const nowTs = Math.floor(Date.now() / 1000);
  const cotisationExpired = membershipEnd !== null && membershipEnd < nowTs;
  const subscriptionExpired =
    subscriptionEnd !== null && subscriptionEnd < nowTs;
    ...
    <Box sx={{ p: 3 }}>
      ...
      {noData && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Aucun historique de paiement trouvé. Contactez{" "}
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    <TableRow key={inv.id}>
                      ...
                      <TableCell>
                        {formatDate(inv.date_lim_reglement)}
                      </TableCell>
            ...
            {canPayAdvance ? (
              ...
              >
                ...
                <Typography>
                  Total :{" "}
                  {formatAmount(
                    Math.min(advanceMonths, maxAdvanceMonths) *
                      monthlyPrice(
                        user.membership.type,
              ...
              <Alert severity="success">
                Vous êtes à jour pour l'année — aucun mois supplémentaire
                disponible.
              </Alert>
            )}
          </>
