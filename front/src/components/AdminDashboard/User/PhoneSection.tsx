...
export default function PhoneSection({
  ...
  const [phoneVerifiedToUpdate, setPhoneVerifiedToUpdate] = useState<boolean>(
  ...
  const onUpdatePhone = async () => {
    ...
    if (PhoneToUpdate) {
      ...
      if (!validation) {
        alert(
          "Mauvais format pour le numéro de téléphone, utiliser le format internationnal (+33 pour la France par ex.)",
        );
        return;
      }
    }
    ...
    try {
      await Api.updateUser(user.id, {
        ...
        phone_number_verified: phoneVerifiedToUpdate,
      });
      ...
      {(editingPhone || editingVerified) && (
        <Box mt={2}>
          <ConfirmableButton
            ...
            confirmationText={
              <p>Les informations du téléphone vont être mises à jour.</p>
            }
          >
