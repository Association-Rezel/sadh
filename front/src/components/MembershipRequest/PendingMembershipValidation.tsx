...
export default function PendingMembershipValidation({
  user,
}: {
  user: User;
}): JSX.Element {
  if (user.membership.type === MembershipType.FTTH) {
    ...
    return <Alert severity="error">Type d'adhésion inconnu</Alert>;
  }
}
...
function DolibarrPayment({ user }: { user: User }) {
  ...
  if (!user.membership.paid_first_month) {
    if (user.membership.type === MembershipType.FTTH)
      availableItems.push({
        itemId: "first_month",
        displayName: "1er mois Fibre",
        price: ftthMonthly,
      });
    else if (user.membership.type === MembershipType.WIFI)
      availableItems.push({
        itemId: "first_month",
        displayName: "1er mois Wi-Fi",
        price: wifiMonthly,
      });
  }
  if (
  ...
  if (!user.membership.paid_membership) {
    availableItems.push({
      itemId: "membership",
      displayName: "Cotisation annuelle",
      price: 1,
    });
  }

  const [itemsChecked, setItemsChecked] = useState<string[]>(
    availableItems.map((item) => item.itemId),
  );
  ...
  useEffect(() => {
    ...
    const handler = () => {
      navigator.sendBeacon(
        `/api/payments/me/invoices/${pendingInvoiceId}/cancel`,
      );
    };
  ...
  if (availableItems.length === 0) {
    return (
      <Alert severity="success">Tous les paiements ont été effectués !</Alert>
    );
  }

  return (
    <div className={"flex flex-col gap-2 sm:items-start items-stretch"}>
      <Typography
        variant="body1"
        color="text.secondary"
        component="p"
        align="left"
      >
        Sélectionne les paiements que tu souhaites réaliser, puis clique sur le
        bouton pour être redirigé vers la page de paiement sécurisée.
      </Typography>
      <Paper elevation={2}>
        <PurchaseSelect
      ...
      {!opened ? (
        ...
        <>
          <Typography variant="caption" color="error" display="block">
            ⚠️ Si tu annules ou rafraîchis cette page, ferme tous les onglets de
            paiement ouverts.
          </Typography>
          <Button
            variant="outlined"
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            La facture va être annulée.
            <br />
            <br />
            <strong>Important :</strong> l'onglet de paiement va être fermé
            automatiquement.
            <br />
            <br />
            Si il est toujours ouvert, fermez-le manuellement !<br />
            <br />
            Si vous payez sur un onglet après annulation, contactez rapidement{" "}
            <a href="mailto:support@rezel.net">support@rezel.net</a>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      {cancelError && (
        <Alert severity="error">
          Impossible d'annuler la facture. Ferme l'onglet de paiement
          manuellement pour éviter un paiement accidentel, puis réessaie.
        </Alert>
      )}
      {opened && polling && (
        <Alert severity="info" icon={<CircularProgress size="1.2em" />}>
          Paiement ouvert dans un nouvel onglet.{" "}
          <strong>Ferme ce dernier si tu ne souhaites pas payer.</strong>
          <br />
          En attente de la confirmation...
        </Alert>
      )}
      {opened && !polling && (
        <Alert severity="success">Paiement confirmé !</Alert>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4 items-start">
      <Typography
        ...
        align="left"
      >
        En adhérant à Rezel, tu t'engages à respecter le règlement intérieur de
        l'association.
      </Typography>
    ...
    <>
      ...
      <div className="flex flex-col gap-8">
        ...
        >
          ...
          <span className="ml-2">Contrat de fourniture de service</span>
        </Typography>
        ...
        <div className={"flex flex-col gap-2"}>
          ...
          <div className={"pl-4 flex flex-col gap-4"}>
            ...
            <Typography
              ...
              align="left"
            >
              La caution vous sera restituée à la fin de votre adhésion en
              échange des équipements fournis.
            </Typography>

            <PaymentStatus
              paid={user.membership.paid_first_month}
              display={
                user.scholarship_student
            ...
            >
              ...
              <br />
              Votre abonnement ne commencera qu'à partir de la date de votre
            ...
            <div>
              ...
              <Typography
                ...
                align="left"
              >
                Vous devez régler votre cotisation pour un an. Être membre
                adhérent de Rezel est obligatoire pour bénéficier du service
                FAI.
                <br />
                Votre adhésion commencera le jour de la signature de votre
                contrat.
                <br />
                Elle est à renouveler chaque année à la date anniversaire de
                votre adhésion.
              </Typography>
            </div>
          </div>
        ...
        <Typography
          ...
          align="justify"
        >
          Nous t'enverrons un mail dès que les paiements auront été vérifiés.
          Ensuite, tu pourras récupèrer tes équipements (Box Internet), et nous
          te demanderons un créneau pendant lequel tu es disponible pour qu'un
          technicien mandaté par Orange procède à l'installation de la fibre.
          Cette étape est obligatoire car les infrastructures FTTH (Fibre To The
          Home) sont mutualisées. Ton adhésion commencera le jour de
          l'installation de la fibre, et nous te demanderons de régler ton
          abonnement de {user.scholarship_student ? "15€" : "20€"} tous les mois
          à partir de ce moment.
          <br />
          <br />
          Si tu as la moindre question, n'hésites pas à paser au local de
          l'association, ou bien à envoyer un mail à{" "}
          <a href="mailto:fai@rezel.net">fai@rezel.net</a>
        </Typography>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className={"flex flex-col gap-2"}>
          ...
          <div className={"pl-4 flex flex-col gap-4"}>
            ...
            <div>
              ...
              {!user.membership.paid_membership && (
                <Alert className={"text-left"} severity={"warning"}>
                  Nous ne vérifions pas automatiquement que la cotisation a été
                  prise en compte. Si tu as déjà cotisé à l'association, il
                  n'est pas nécessaire de le faire de nouveau
                </Alert>
              )}
            </div>
          </div>
        ...
        >
          Nous t'enverrons un mail dès que les paiements auront été vérifiés.
          Ensuite, tu recevras un mail te confirmant qu'un nouveau réseau Wi-Fi
          a été crée pour toi, et nous te demanderons de régler ton abonnement
          de {user.scholarship_student ? "5€" : "10€"} tous les mois à partir de
          ce moment.
          <br />
          <br />
          Si tu as la moindre question, n'hésites pas à paser au local de
        ...
        <Accordion>
          ...
          >
            <Typography
              ...
              className="flex items-center gap-2"
            >
              <PaymentsIcon /> Par chèque ou espèce
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <ChequePaymentInfo />
        ...
        <Accordion>
          ...
          >
            <Typography
              ...
              className="flex items-center gap-2"
            >
              <AccountBalanceIcon /> Par virement bancaire
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <VirementPaymentInfo />
  ...
  if (user.membership.contract_signed)
    return (
      <Typography
        ...
        align="left"
      >
        Contrat signé reçu ! Tu recevras par mail une copie du contrat signé par
    ...
    return (
      <div className="flex flex-col gap-4 items-start">
        <Typography
          ...
          align="left"
        >
          Un mail t'a été envoyé avec le contrat de fourniture de service
          pré-rempli. <br />
          Merci de le signer et de le renvoyer à{" "}
          <a href="mailto:fai@rezel.net">fai@rezel.net</a>.
        </Typography>
      </div>
    );
...
function ChequePaymentInfo() {
  return (
    ...
    >
      ...
      Dans ce cas, merci de passer au local de l'association, qui se trouve en
      salle 0A316 à Télécom Paris (19 Place Marguerite Perey). <br />
      <br />
      Pour s'assurer de la présence d'un bénévole, envoie nous un mail à{" "}
      <a href="mailto:fai@rezel.net">fai@rezel.net</a> en précisant à quelle
      heure tu souhaites passer.
    </Typography>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4 items-start">
      ...
      >
        ...
        ⚠️ Dans ce cas, tu dois impérativement mentionner{" "}
        <strong>ton nom et prénom dans le libellé du virement.</strong>
        <br />
      </Typography>
      <Button
        target="_blank"
