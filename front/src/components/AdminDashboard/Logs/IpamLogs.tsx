...
export default function IpamLogs() {
  ...
  const filteredLogs = logs?.filter((log) =>
    log.message.includes(currentSearchString),
  );
  ...
  const handleSubmitCreateLog = async () => {
    try {
      await Api.createIpamLog(
        createLogForm.getValues().message,
        createLogForm.getValues().source,
      );
      loadLogs();
    } catch (error) {
      alert(error.message);
    ...
    <div className="flex flex-col gap-8">
      <div className="col-span-3 flex flex-col mt-16">
        ...
        <Typography variant="subtitle1" color="text.secondary" align="left">
          Peut être utile pour des actions manuelles sur la base de données ou
          pour ajouter du contexte sur une ressource à un instant donné.
        </Typography>
        <form
          className="grid grid-cols-1 md:flex flex-row gap-12 items-end min-w-max"
          onSubmit={createLogForm.handleSubmit(handleSubmitCreateLog)}
        >
          <Controller
            name="source"
          ...
          <Button
            type="submit"
            variant="outlined"
            disabled={createLogForm.formState.isSubmitting}
            sx={{ minWidth: "10rem" }}
          >
            Créer ⚠️
          </Button>
      ...
      <div>
        ...
        <Typography variant="subtitle1" color="text.secondary" align="left">
          Rechercher (côté client, dans le navigateur) une chaine de caractères
          dans les logs
        </Typography>
      </div>
      ...
      {loading || createLogForm.formState.isSubmitting ? (
        <CircularProgress />
      ) : null}
      {filteredLogs && (
        <>
