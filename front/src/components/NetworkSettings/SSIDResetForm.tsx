...
export default function SsidResetForm({ unet, setUnet }) {
  ...
  async function isSSIDValid() {
    if (newSSID.length < 10 - 6) {
      return {
        valid: false,
        message: "Le SSID doit contenir au moins 10 caractère",
      };
    } else if (newSSID.length > 32 - 6) {
      return {
        valid: false,
        message: "Le SSID doit contenir au plus 32 caractère",
      };
    } else {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!validSSID.valid) {
      ...
      try {
        ...
        setInfo(
          "Le SSID a bien été modifié, la modification sera effective à 6h du matin",
        );
      } catch (apiError) {
        setError(
          apiError.message ||
            "Une erreur est survenue lors de la mise à jour du SSID",
        );
      }
    }
    setLoading(false);
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          <TextField
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">Rezel-</InputAdornment>
              ),
            }}
            value={newSSID}
          ...
          <Button
            ...
            type="submit"
            disabled={
              newSSID === unet.wifi.ssid.replace("Rezel-", "") || loading
            }
          >
