...
export default function ONTSection({
  ...
  const onSubmit: SubmitHandler<RegisterONT> = (register: RegisterONT) => {
    ...
    setONTLoading(true);
    Api.registerONT(user_id, register)
      .then((ont) => {
        setONT(ont);
      })
      .catch((e) => {
  ...
  const onDelete = () => {
    setONTLoading(true);
    Api.deleteONT(ont.serial_number)
      .then(() => {
        setONT(null);
      })
      .catch((e) => {
  ...
  const onForceRegisterInOLT = () => {
    setONTLoading(true);
    Api.forceOntRegistration(ont.serial_number)
      .catch((e) => {
  ...
  return (
    <div className="mt-10 max-w-xs">
      ...
      >
        ...
        {!ontLoading && ont && (
          <>
            <strong>Numéro de série</strong> : {ont.serial_number}
            <br />
            <strong>Position au PM</strong> : {ont.mec128_position}
            <br />
            <strong>PON Interface</strong> : {ont.olt_interface}
            <br />
            <strong>PM</strong> : {ont.pm_description}
            <br />
            <strong>Position porte droite</strong> :{" "}
            {ont.position_in_subscriber_panel}
            <br />
            <Typography
              variant="h6"
              align="left"
            ...
            {ont.configured_in_olt == null && (
              <Alert severity="warning">
                Aucune donnée remontée par net-adh-scraper. Soit l'ONT n'est pas
                enregistré dans l'OLT, soit il faut attendre la prochaine
                éxécution de net-adh-scraper (quelques minutes).
              </Alert>
            )}
            {ont.configured_in_olt != null && !ont.configured_in_olt && (
              <Alert severity="error">
                L'ONT n'est pas enregistré dans l'OLT. Vous pouvez forcer
            ...
            {ont.configured_in_olt &&
              (() => {
                const lastFetchedDate = new Date(
                  ont.operational_data.last_fetched * 1000,
                );
                const isDataObsolete =
                  new Date().getTime() - lastFetchedDate.getTime() >
                  6 * 60 * 60 * 1000;
                  ...
                  <>
                    ...
                    {isDataObsolete && (
                      <Alert severity="warning" sx={{ my: 2 }}>
                        Attention : La supervision semble obsolète. La dernière
                        mise à jour des données date du{" "}
                        {lastFetchedDate.toLocaleString("fr-FR")}.
                      </Alert>
                    )}
                    au{" "}
                    {lastFetchedDate.toLocaleDateString(navigator.language, {
                      hour: "numeric",
                      minute: "numeric",
                    })}
                    <br />
                    ...
                    {ont.operational_data?.admin_status && (
                      <>
                        ...
                        {ont.operational_data.last_operational_up
                          ? new Date(
                              ont.operational_data.last_operational_up * 1000,
                          ...
                          : "Jamais"}
                        <br />
                        <strong>Réception signal (dBm)</strong> :{" "}
                        {ont.operational_data.dbm_level}
                        <br />
                        <strong>Distance estimée</strong> :{" "}
                        {ont.operational_data.estimation_distance} km
                        <br />
                        <strong>OLT path</strong> : {ont.operational_data.path}{" "}
                        <br />
                      </>
                    )}
                    {!ont.operational_data?.admin_status && (
                      <Alert severity="warning">
                        L'ONT est désactivé administrativement dans l'OLT. Pour
                        ré-activer, vous pouvez forcer le ré-enregistrement
                        ci-dessous
                      </Alert>
                    )}
                    <br />
                    {!ont.operational_data && (
                      <Chip
                        color="error"
                        label="Erreur. Etat innatendu (pas de données opérationelles)."
                      />
                    )}
                  </>
                );
            ...
            <Button variant="text" size="small" onClick={onForceRegisterInOLT}>
              Forcer ré-enregistrement OLT
            </Button>
          </>
        )}
      </Typography>
