...
export default function StatusUpdateSection({
  ...
  useEffect(() => {
    Api.fetchNextMembershipStatus(user.id)
      .then((update) => {
  ...
  const onUpdate = (status: MembershipStatus) => {
    setUpdating(true);
    Api.updateMembershipStatus(user.id, status)
      .then((updatedUser) => {
        if (updatedUser == null) {
      ...
      .catch((error) => {
        alert(
          "Erreur lors de la modification. Message d'erreur : " + error.message,
        );
      })
      .finally(() => {
        setUpdating(false);
  ...
  if (!loading && !nextUpdate)
    return (
      <div className="mt-10 max-w-xs">Aucun changement d'état possible</div>
    );
    ...
    <div className="mt-10 max-w-xs">
      ...
      >
        <div className="flex flex-col gap-y-2">
          ...
          {nextUpdate &&
            nextUpdate.conditions.map((condition) => (
              <span key={condition}>
                {nextUpdate.conditions_not_met.includes(condition) ? (
                  <Close color="error" />
                ) : (
                  <Check color="success" />
                )}{" "}
