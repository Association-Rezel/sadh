...
export default function AppointmentSelection({
  ...
  useEffect(() => {
    Api.fetchAppointmentSlots(weekOffset)
      .then((data) => {
        if (data === null) {
          alert(
            "Erreur lors de la récupération des créneaux. Veuillez essayer de recharger la page.",
          );
          return;
        }
        setSlots(data);
      ...
      .catch((error) => {
        alert(
          "Erreur lors de la récupération des créneaux. Veuillez essayer de recharger la page. Message d'erreur : " +
            error.message,
        );
      });
  }, [weekOffset]);
    ...
    <div className="mt-10 flex flex-col items-center">
        ...
        {slots.map((day, index) => (
          <div className="flex flex-col gap-6" key={index}>
            <Typography variant="h6">
              {day[0].start.toLocaleDateString("fr-FR", {
                weekday: "long",
                day: "numeric",
                month: "long",
              })}
      ...
      <div className="max-w-2xl mt-12 mx-4">
        <Typography variant="body1" align="justify">
          ...
          selon les disponibilités des membres de Rezel et des techniciens
          Orange.
          <br />
          <br />
          Le technicien peut se présenter à n'importe quel moment durant le
          créneau, et l'installation peut durer jusqu'à une heure, même si cela
          déborde sur la fin du créneau.
        </Typography>
      </div>
      <div className="flex flex-row gap-4 justify-center my-8">
        <Button
          ...
          onClick={() => onSubmitSelection(selectedSlots)}
        >
          Envoyer
        </Button>
      </div>
