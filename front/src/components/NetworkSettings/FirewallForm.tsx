...
export function AddIPv4RedirectionForm({
  unet,
  setUnet,
}: {
  unet: UnetProfile;
  setUnet: (value: UnetProfile) => void;
}) {
  ...
  const isValidName = (name: string) => {
    ...
    if (name.length < 3) {
      ...
      unet.firewall.ipv4_port_forwarding.some(
        (redirection) => redirection.name.trim() === name.trim(),
      )
    ) {
  ...
  const addRedirection = async (data: FormData) => {
    ...
    if (isNaN(wan_port) || wan_port < 0 || wan_port > 65535) {
      setErrors((prev) => ({
        ...prev,
        wan_port: "Le port doit être un entier compris entre 0 et 65535",
      }));
      return false;
    }
    ...
    if (isNaN(lan_port) || lan_port < 0 || lan_port > 65535) {
      setErrors((prev) => ({
        ...prev,
        lan_port: "Le port doit être un entier compris entre 0 et 65535",
      }));
      return false;
    }
    ...
    if (!lan_ip) {
      setErrors((prev) => ({
        ...prev,
        lan_ip: "L'adresse IP locale est obligatoire",
      }));
      return false;
    }

    if (lan_ip.slice(0, 9) !== unet.network.lan_ipv4.address.slice(0, 9)) {
      const network_address = unet.network.lan_ipv4.address.replace(
        /192\.168\.(\d+).1+\/24+/,
        "192.168.$1.0/24",
      );
      setErrors((prev) => ({
        ...prev,
        lan_ip: "L'adresse IP locale doit être dans réseau " + network_address,
      }));
      return false;
    }
    ...
    if (protocol !== "tcp" && protocol !== "udp") {
      setErrors((prev) => ({
        ...prev,
        protocol: "Le protocole doit être TCP ou UDP",
      }));
      return false;
    }
    ...
    try {
      ...
      setErrors({
        name:
          apiError.message ||
          "Une erreur est survenue lors de l'ajout de la redirection de port",
      });
      return false;
    }
  ...
  return (
    <PopupForm
      ...
      startIcon={<AddIcon />}
    >
      <Stack direction={"column"} spacing={2} width={"100%"} marginTop={2}>
        <TextField
          name="name"
          label="Nom"
          variant="outlined"
        ...
        />
        <FormControl required error={!!errors.protocol}>
...
export function AddIPv6OpeningForm({
  unet,
  setUnet,
}: {
  unet: UnetProfile;
  ...
  const isValidName = (name: string) => {
    ...
    if (name.length < 3) {
      ...
      unet.firewall.ipv6_port_opening.some(
        (opening) => opening.name.trim() === name.trim(),
      )
    ) {
      return { valid: false, message: "Le nom est déjà utilisé" };
    }
  ...
  const addOpening = async (data: FormData) => {
    ...
    if (isNaN(port) || port < 0 || port > 65535) {
      setErrors((prev) => ({
        ...prev,
        port: "Le port doit être un entier compris entre 0 et 65535",
      }));
      return false;
    }
    ...
    try {
      ...
      setErrors({
        name:
          apiError.message ||
          "Une erreur est survenue lors de l'ajout de l'ouverture de port",
      });
      return false;
    }
